...
def pose_regressor_stack(image_star, images, sc=2):
    ...
    images = tf.reshape(tf.transpose(images,
        [0, 2, 3, 1, 4]), [batch, height, width, -1])

    with tf.device('/cpu:0'):
        inputs = tf.image.resize_area(images/255.0, [int(height/sc), int(width/sc)])
...
def pose_regressor_indv(image1, image2, sc=2):
    batch, height, width, _ = image1.get_shape().as_list()
    inputs = tf.concat([image1/255.0, image2/255.0], axis=-1)

    with tf.device('/cpu:0'):
        inputs = tf.image.resize_area(inputs, [int(height/sc), int(width/sc)])
    ...
    with tf.variable_scope('pose') as sc:
        with slim.arg_scope([slim.conv2d],
            ...
            pose_vec = 0.01*tf.reshape(pose_vec, [batch, 6])

            G = pose_vec2mat(pose_vec)
            return G


def pose_regressor_factory(image_star, images, intrinsics, cfg):

    batch, frames, height, width, _ = images.get_shape().as_list()
    image_dims = [batch*frames, height, width, -1]
    image1 = tf.reshape(tf.tile(image_star, [1, frames, 1, 1]), image_dims)
